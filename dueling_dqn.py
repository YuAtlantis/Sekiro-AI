...
class DQNAgent:
    ...
    def load_checkpoint_or_model(self):
        ...
        if os.path.exists(checkpoint_dir) and os.path.isdir(checkpoint_dir):
            ...
            if checkpoints:
                ...
                self.eval_net.load_state_dict(torch.load(checkpoint_path, map_location=device))

                step_path = os.path.join(self.model_folder, "last_step.txt")
    ...
    def log_metrics(self, loss, reward_batch, q_values, total_norm):
    ...
    def train(self, batch_size, step):
        ...
        state_batch = torch.tensor([data[0] for data in minibatch], dtype=torch.float32).to(device)
        ...
        next_state_batch = torch.tensor([data[3] for data in minibatch], dtype=torch.float32).to(device)
        ...
        self.log_metrics(loss, reward_batch, q_values, total_norm)
    ...
    def save_checkpoint(self, step):
        checkpoint_path = os.path.join(self.model_folder, f"checkpoint_step_{step}.pth")
        ...
        checkpoints = [f for f in os.listdir(self.model_folder) if f.startswith("checkpoint_step_") and f.endswith('.pth')]
